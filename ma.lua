...
--// Main Instances \\-
-- Cache and lock down originals to prevent tampering
...
--- Safely parents a GUI, protecting it first if possible.
-- @param UI Instance -- the GUI to parent
-- @param SkipHiddenUI boolean? -- skip hidden-parent path
local function ParentUI(UI, SkipHiddenUI)
    -- 1) Validate input
    assert(typeof(UI) == "Instance", "ParentUI: UI must be an Instance")

    -- 2) If user opts out of hidden parenting, do the normal path
    if SkipHiddenUI then
        originalSafeParentUI(UI, CoreGui)
        return
    end

    -- 3) Try the strongest protection available
    local ok, err = pcall(function()
        if syn and syn.protect_gui then
            syn.protect_gui(UI)
        elseif originalProtectGUI then
            originalProtectGUI(UI)
        else
            -- No protection API; consider alternative hardening here
        end
    end)

    -- 4) Log failures so we know if GUI remains unprotected
    if not ok then
        warn(("ParentUI: GUI protection failed – %s"):format(tostring(err)))
    end

    -- 5) Finally parent into the hidden UI context
    originalSafeParentUI(UI, HIDDEN_UI)
end
